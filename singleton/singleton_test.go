...
// Singletonは唯一のインスタンスを用意し、それ以外を生成できなくするパターン
// そのため、唯一のインスタンスしか存在しないことを保証できる
...
// 逆に一般的な生成パターンは、コンストラクタ関数の中でインスタンスを生成する
// そのため、同じ値を持ちますが(同値)、参照は同じ(同一)ではない
...
// InstanceByOnce() はロード時にインスタンスを生成しておくのではなく、
// InstanceByOnce() がコールされた時点で必要に応じて生成する形式の Singleton実装。同期処理を sync.Onceで実現している。
func TestSingletonByOnce(t *testing.T) {
	s1 := singleton.InstanceByOnce()
	s2 := singleton.InstanceByOnce()
